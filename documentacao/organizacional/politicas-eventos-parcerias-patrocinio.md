# Politicas de Eventos, Parcerias e Patrocinio

## Politica de Eventos

### Tipos de eventos

- Meetup local.
- Workshop tecnico.
- Live ou aula online.
- Hackathon.
- Painel com empresas.
- Encontro de networking.
- Conferencia FullDev.

### Regras obrigatorias

- Todo evento deve seguir o codigo de conduta.
- Deve haver responsavel definido.
- Deve haver agenda basica.
- Deve haver comunicacao clara sobre local, horario, vagas, gravacao e patrocinadores.
- Eventos presenciais devem ter plano minimo de check-in, seguranca e acessibilidade.
- Palestrantes devem aceitar diretrizes de conduta.
- Eventos pagos precisam de aprovacao nacional.
- Cobranca de ingresso precisa de aprovacao da FullDev Central antes da divulgacao.
- Taxas para reembolso de custos, como local, coffee break ou materiais, tambem precisam de aprovacao previa.
- Eventos pagos devem informar claramente preco, destino do valor, politica de reembolso e eventuais patrocinadores.

### Checklist minimo

- Objetivo do evento.
- Publico-alvo.
- Data e local.
- Responsavel.
- Palestrantes.
- Parceiros.
- Arte aprovada.
- Link de inscricao.
- Lista de presenca.
- Registro pos-evento.
- Feedback.

## Politica de Parcerias

### Parceiros possiveis

- Empresas de tecnologia.
- Universidades e escolas.
- Coworkings.
- Comunidades amigas.
- Organizadores de eventos.
- Plataformas educacionais.
- Recrutadores e consultorias.

### Criterios

- Alinhamento com a cultura FullDev.
- Beneficio claro para membros.
- Transparencia sobre interesses comerciais.
- Ausencia de praticas abusivas.
- Respeito a dados e privacidade.
- Nao exclusividade sem aprovacao nacional.

### Contrapartidas possiveis

- Espaco para evento.
- Palestrantes.
- Brindes.
- Divulgacao de vagas.
- Mentorias.
- Descontos.
- Patrocinio financeiro.
- Infraestrutura.

## Politica de Patrocinio

### Principios

Patrocinio deve sustentar a comunidade, nao capturar sua independencia.

### Categorias

- Local: apoia evento ou nucleo regional especifico.
- Nacional: apoia iniciativas amplas da FullDev Central.
- Educacional: apoia aulas, trilhas ou bolsas.
- Produto/plataforma: apoia ferramentas ou infraestrutura.
- Conferencia: apoia evento principal anual.

### Regras

- Toda proposta com dinheiro deve ser registrada.
- Contrapartidas devem ser claras.
- Patrocinador nao pode interferir em moderacao, conteudo editorial ou decisoes de comunidade.
- Leads e dados de membros nao podem ser vendidos.
- Sorteios e promocoes devem ter regras claras.
- Patrocinios com exclusividade exigem Conselho Nacional.

## Quem decide

- Parcerias nacionais: FullDev Central decide.
- Patrocinios nacionais: FullDev Central decide.
- Parcerias locais: decisao conjunta; nucleo propoe e Central aprova.
- Patrocinios locais: decisao conjunta; nucleo pode prospectar, mas Central aprova valores, contrapartidas, uso de marca e comunicacao.
- Busca de patrocinador proprio por nucleo: nucleo pode iniciar conversa, mas nao pode fechar, anunciar ou assinar sem aprovacao da Central.
- Conflito com patrocinador nacional: FullDev Central decide. Patrocinio local nao pode conflitar com patrocinador nacional, exclusividade, categoria protegida ou posicionamento institucional.
- Uso de marca por parceiro: FullDev Central decide.
- Compartilhamento de dados ou leads: FullDev Central decide, com regra padrao de nao venda de dados de membros.

## Uso de fundos

- Fundos nacionais sao geridos pela FullDev Central.
- Fundos locais podem ser propostos pelo nucleo, mas o uso precisa de aprovacao da Central.
- Todo uso de recurso deve ter finalidade clara, valor estimado, responsavel e comprovante.
- Reembolso para voluntarios precisa de aprovacao previa e comprovacao.
- A Central define modelo de prestacao de contas, prazos e nivel de transparencia interna.
- Recursos obtidos por ingresso, patrocinio local ou apoio devem ser registrados antes de uso.

## Apoios e parcerias atuais

### HostGator

A HostGator possui acordo firmado de parceria com a FullDev em formato de acordo de cavalheiros.

A comunicacao pode tratar a HostGator como parceira da FullDev, desde que nao apresente a relacao como patrocinio formal assinado quando nao houver contrato especifico.

### Asaas

A Asaas possui acordo de apoio firmado para eventos da FullDev, ainda sem contrato assinado.

A comunicacao publica deve usar linguagem conservadora:

- Correto: "apoiadora de eventos", "apoio a eventos da FullDev" ou "apoio institucional a eventos".
- Evitar: "patrocinadora oficial", "parceria contratual" ou "apoio assinado", enquanto nao houver contrato formal.

O apoio da Asaas para a FullDev School ainda depende de nova conversa e definicao especifica.

### Eventos e apoios pontuais

- Codecon: primeiro evento em que a FullDev participara gratuitamente dentro do contexto de apoio a eventos.
- Web Summit Rio: a FullDev conseguiu 10 ingressos por meio da equipe do Web Summit. Esse registro deve ser tratado como cortesia/apoio pontual do evento, nao como parceria institucional ampla.

Antes da divulgacao, registrar:

- O que cada parceiro oferece.
- Quais contrapartidas a FullDev assume.
- Como a marca sera usada.
- Se havera coleta de dados.
- Quem aprova comunicados.
- Prazo de vigencia do apoio.

## Motivo da recomendacao

Eventos e parcerias sao alavancas de crescimento, mas tambem fontes de risco reputacional e conflito de interesse. A politica permite captar apoio sem perder independencia.

## Beneficios, riscos e trade-offs

- Beneficio: facilita relacionamento com empresas e universidades.
- Beneficio: protege membros de venda abusiva e coleta indevida de dados.
- Risco: regras podem afastar patrocinadores agressivos comercialmente.
- Trade-off: a FullDev deve preferir parceiros alinhados, mesmo que isso reduza receita de curto prazo.
