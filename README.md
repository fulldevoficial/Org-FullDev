# Central de Documentação FullDev

Este repositório centraliza a documentação da FullDev: história, manifesto, governança, projetos, decisões organizacionais e materiais de apoio para membros, lideranças, parceiros e colaboradores.

A FullDev é uma comunidade criada por desenvolvedores para desenvolvedores, com foco em networking, aprendizado prático, oportunidades, eventos, projetos colaborativos e impacto de carreira.

> By Dev, For Dev.

[Acesse o site da FullDev](https://www.fulldev.com.br)

## Como usar este repositório

- Use a documentação básica para entender a origem, a cultura e a proposta inicial da comunidade.
- Use a documentação organizacional como referência estratégica atual para governança, expansão, papéis, processos e tomada de decisão.
- Use os documentos de projetos para consultar iniciativas ativas, propostas em validação e registros históricos.
- Quando houver divergência entre documentos antigos e a documentação organizacional, trate o ponto como decisão a ser validada pelo Conselho Central.

## Documentação principal

### Documentação básica

1. [História da FullDev](documentacao/doc-basica/historia.md)
2. [O que é a FullDev?](documentacao/doc-basica/manifesto.md)
3. [Cartilha do Moderador](https://docs.google.com/document/u/0/d/1Efc1rOrpzsbFTUwfEb_Qm4hkM6oYiWd6-7nB7xTVddA/mobilebasic)
4. [Cartilha do Conselho](https://docs.google.com/document/d/1WeIo4K9adwOIpCtjsUUwD9yYR4mJfh3gNckJyXkH8kA/edit?tab=t.0#heading=h.s53l839phq4u)
5. [Organograma](https://www.figma.com/design/v3vRWEqqiVjqTK1vpbLpRF/Organograma-FullDev?node-id=0-1&t=3BQjgE0P255Qu7s5-1)

### Documentação organizacional

1. [Visão geral da documentação organizacional](documentacao/organizacional/README.md)
2. [Manifesto, Missão, Visão e Valores](documentacao/organizacional/manifesto-missao-visao-valores.md)
3. [Estrutura Organizacional e Organograma](documentacao/organizacional/estrutura-organizacional.md)
4. [Equipe FullDev Central](documentacao/organizacional/equipe-fulldev-central.md)
5. [Equipe FullDev School](documentacao/organizacional/equipe-fulldev-school.md)
6. [Estratégia do Discord](documentacao/organizacional/estrategia-discord.md)
7. [Comunidades Parceiras](documentacao/organizacional/comunidades-parceiras.md)
8. [Guia de Eventos](documentacao/organizacional/guia-de-eventos.md)
9. [Estratégia de Aquisição de Membros](documentacao/organizacional/estrategia-aquisicao-membros.md)
10. [Estratégia de Instagram](documentacao/organizacional/estrategia-instagram.md)
11. [Modelo de Governança](documentacao/organizacional/governanca.md)
12. [Código de Conduta](documentacao/organizacional/codigo-de-conduta.md)
13. [Manual dos Líderes Regionais](documentacao/organizacional/manual-lideres-regionais.md)
14. [Criação de Núcleos Regionais](documentacao/organizacional/criacao-de-nucleos-regionais.md)
15. [Portfólio Atual e Status dos Projetos](documentacao/organizacional/portfolio-atual.md)
16. [FullDev School e Projeto de Extensão PUC Minas](documentacao/organizacional/fulldev-school-projeto-extensao.md)
17. [Roteiro de Perguntas e Decisões](documentacao/organizacional/roteiro-perguntas-decisoes.md)
18. [Políticas de Eventos, Parcerias e Patrocínio](documentacao/organizacional/politicas-eventos-parcerias-patrocinio.md)
19. [Playbook Operacional e Comunicação Interna](documentacao/organizacional/playbook-operacional-comunicacao.md)
20. [Tomada de Decisão e Escalabilidade](documentacao/organizacional/decisao-escalabilidade.md)

## Equipe atual da FullDev Central

Composição operacional registrada em [equipe-fulldev-central.md](documentacao/organizacional/equipe-fulldev-central.md). Uma mesma pessoa pode acumular funções, mas toda área precisa ter responsável, escopo, rotina e canal com a Central.

| Área | Responsáveis |
| --- | --- |
| Fundação, Parcerias e Direção da School | Lucas Pedro |
| Decisor na ausência do fundador | Matheus Braz |
| Tech Advisor da comunidade | Carlos |
| Conselho Central e Coordenação Geral | Lucas Pedro, Matheus Braz, Carlos, Lyssa |
| Comunidade WhatsApp | Gabu, Matheus Braz |
| Comunidade Discord | João |
| Comunicação e Marca | Estagiário da PUC |
| Eventos | Lyssa |
| Parcerias | Lucas Pedro |
| Operações | Hernando |
| Técnico / Plataforma | Inteligência artificial, Carlos (Tech Advisor), João |
| Financeiro / Administrativo | A definir (apoio pontual de Carlos, sem assumir a área) |

O documento detalha ainda o modelo sugerido para Núcleos Regionais (owner regional, co-owner, comunidade, comunicação, eventos, parcerias e operações locais) e a equipe de suporte da FullDev School (direção, coordenação pedagógica, instrutores, hosts, suporte aos alunos, comunicação, operações, técnico/transmissão e parcerias educacionais).

Pendências em aberto: responsável financeiro/administrativo, processo de seleção do estagiário de Comunicação e Marca e critérios finais para escolha de owners regionais.

## Status dos projetos

### Ativos

| Projeto | Status | Documentação |
| --- | --- | --- |
| FullDev School | Ativo | [Abrir documentação](documentacao/fullDev-School/readme.md) |
| FullDev Links | Ativo | [Abrir documentação](documentacao/fullDev-Links/readme.md) |

### Parados ou não confirmados como ativos

Estas iniciativas aparecem no histórico do repositório, mas precisam de validação sobre continuidade, responsável, próximos passos e rotina operacional.

| Projeto | Status | Documentação |
| --- | --- | --- |
| FullDev Games | Parado | [Abrir documentação](documentacao/fullDev-Games/readme.md) |
| FullDev Strong | Parado | Sem documento próprio no repositório |

### Comunidades Parceiras

Comunidades independentes, com governança própria, que mantêm acordo de cooperação com a FullDev. Não são frentes da FullDev. Detalhes em [comunidades-parceiras.md](documentacao/organizacional/comunidades-parceiras.md).

| Comunidade | Status | Observação |
| --- | --- | --- |
| Queens of Deploy (historicamente CodeQueens) | Comunidade Parceira | Lideranças não compõem o Conselho Central. |
| RainbowStack | Comunidade Parceira | Lideranças não compõem o Conselho Central. |

### Projetos históricos

Projetos desativados ficam arquivados em `documentacao/projetos inativos/` como registro histórico, não como frentes ativas.

| Projeto | Status | Documentação |
| --- | --- | --- |
| FullDev Online | Desativado | [Abrir documentação](documentacao/projetos%20inativos/fullDev-Online/readme.md) |
| FullDev Atta | Desativado | [Abrir documentação](documentacao/projetos%20inativos/fullDev-Atta/readme.md) |
| FullDev Meetup | Desativado | [Abrir documentação](documentacao/projetos%20inativos/fullDev-meetup/readme.md) |
| FullDev Meruem | Desativado | [Abrir documentação](documentacao/projetos%20inativos/FullDev%20Meruem/README.md) |

## Foco atual

De acordo com a documentação organizacional, a prioridade atual da FullDev é:

1. Consolidar a FullDev School.
2. Organizar a página de links como porta de entrada oficial.
3. Estruturar governança, parcerias e comunicação.
4. Preparar Núcleos Regionais apenas depois que os processos básicos estiverem operando com clareza.

## Como participar

Cada projeto possui sua própria dinâmica, mas todos devem seguir a visão geral da FullDev:

> Acelerar carreiras em tecnologia por meio de networking, conhecimento, oportunidades, eventos, projetos colaborativos e experiências reais de mercado.

Para começar, acesse o site oficial, entre nos canais da comunidade e consulte a documentação do projeto ou frente em que deseja contribuir.

[Acesse: FullDev](https://www.fulldev.com.br)
